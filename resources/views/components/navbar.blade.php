<nav class="font-poppins bg-dianne-500 {{-- bg-midnight-500 --}} text-white px-4 w-full flex justify-between lg:justify-evenly items-center h-16">
    ...
    <!-- search -->
    <div id="hidde-form" class="x1 sm:hidden xl:w-3/4 w-3/6 md:flex md:justify-center md:items-center" id="mobs" id="burgerbtn">
        <form action="{{ route('search') }}" method="GET" role="search" class="hidden sm:hidden md:flex lg:w-26 max-w-screen-lg bg-gradient-to-l from-blue-300 to-blue-500 items-center rounded-full shadow-xl md:h-9 p-2">
            @csrf
            <img class="w-9 mb-1 mr-auto" src="../img/magnifier.svg" alt="magnifier" title="magnifier">
            <input class="rounded-l-full w-full py0 px0 bg-gradient-to-l from-blue-300 to-blue-500 text-white leading-tight focus:outline-none h-2 p-2" type="text" placeholder="Search">
    ...
    <div id="myNav" class="overlay h-0 w-full fixed z-10 top-0 left-0 bg-gray-800 no-underline text-gray-600 block duration-75 focus:ring-gray-200 overflow-y-auto right-2">
